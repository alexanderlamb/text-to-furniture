...
def test_minor_trim_zero_loss_swaps_direction():
    """ONE_ZERO pair: the non-zero-loss part should be trimmed, not the zero-loss one."""
    thickness = 6.35

    # Side panel: X-normal at x=200, extends y=0..80, z=0..300
    # The side does NOT extend past the shelf's slab (shelf far face at y≈93.65),
    # so trimming the side against the shelf's plane is a no-op (loss ≈ 0).
    side_profile = PartProfile2D(
    ...
    # Trimming shelf against side's plane removes a small strip (loss ≈ 7.8%).
    shelf_profile = PartProfile2D(
    ...
    # The shelf (non-zero loss) should be trimmed, not the side (zero loss).
    ...
    assert shelf_max_x <= 200.5, (
        f"Shelf should be trimmed to ~200mm, got max_x={shelf_max_x}"
    )

    # Side should be unaffected (still 80x300)
    side_area = side_trimmed.profile.outline.area
    assert side_area >= 23000.0, (
        f"Side should not be trimmed, area={side_area}"
    )

    # Debug should show the ONE_ZERO decision
    one_zero_count = trim_debug.get("minor_one_zero_count", 0)
    assert one_zero_count >= 1, (
        f"Expected minor_one_zero_count >= 1, got {one_zero_count}"
    )
